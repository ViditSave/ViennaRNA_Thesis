...
struct aux_arrays {
  ...
  int **DMLi;  	/* DMLi[j] holds  MIN(fML[i,k]+fML[k+1,j])      */
  	       	/*                MIN(fML[i+1,k]+fML[k+1,j])    */
  	  	/*                MIN(fML[i+2,k]+fML[k+1,j])    */
};
...
PRIVATE INLINE void
clear_aux_arrays( struct aux_arrays *aux,
                  unsigned int      length,
		  unsigned int 	    tile,
		  unsigned int 	    i);
...
{
  unsigned int      *sn;
  int               i, j, ij, length, uniq_ML, *indx, *iindx, *f5, *c, *fML, *fM1;
  int 		    ii, jj, tile, e, e1,e2, arr_read;
  vrna_param_t      *P;
  vrna_md_t         *md;
  ...
  //tile = ((length/30) >> 3) << 3;
  tile = ((300/3) >> 3) << 3;
  //printf("Testing Tile size %d", tile);
  int64_t flop_count = 0;

  /* allocate memory for all helper arrays */
  helper_arrays = get_aux_arrays(length, tile);
  ...
  if (length <= ((fc->strands > 1) ? fc->strands : md->min_loop_size)) {
    /* clean up memory */
    free_aux_arrays(helper_arrays, length, tile);
  ...
  for (ii = length - 1; ii > 0; ii -= tile) {
    for (jj = ii+1; jj < (length + 1); jj += tile) {
      int decide_patch = jj - (ii+1) - tile;
      if (decide_patch < 0) {
 	for (i = MAX_NEW(ii,0); i > MAX_NEW((ii-tile), 0); i -= 1) {
          for (j = MAX_NEW((jj-tile+1),i+1); j < (jj+1); j += 1) {
      
            ij = indx[j] + i;
            //int test_ji = iindx[i] - j;
            //printf("Testing ii and ji %d, %d \n", ij, test_ji);

 	    /* decompose subsegment [i, j] with pair (i, j) */
      	    c[ij] = decompose_pair(fc, i, j, helper_arrays, ms_dat, tile);

      	    /* decompose subsegment [i, j] that is multibranch loop part with at least one branch */
      	    fML[ij] = vrna_E_ml_stems_fast(fc, i, j, helper_arrays->Fmi[i%tile], helper_arrays->DMLi[i%(tile+2)]);
	...
	flop_count = vrna_E_ml_stems_fast_middle (fc, ii, jj, tile, helper_arrays->Fmi, helper_arrays->DMLi, flop_count);    
        ...
        for (i = MAX_NEW(ii,0); i > MAX_NEW((ii-tile), 0); i -= 1) {
  	    ...
  	    /* decompose subsegment [i, j] with pair (i, j) */
      	    ...
      	    /* decompose subsegment [i, j] that is multibranch loop part with at least one branch */
      	    fML[ij] = vrna_E_ml_stems_fast_patch (fc, i, j, ii, jj, tile, helper_arrays->Fmi[i%tile], helper_arrays->DMLi[i%(tile+2)]);
    ...
    clear_aux_arrays(helper_arrays, length, tile, i);
  } /* end of ii-loop */

  //printf("\nFlop Count for middle section is : %ld \n", (long)flop_count);
	       ...
	       int  		    tile)
{
  ...
  DMLi1         = aux->DMLi[(i+1)%(tile+2)];
  DMLi2         = aux->DMLi[(i+2)%(tile+2)];
  cc            = aux->cc[0];
  cc1           = aux->cc[1];
  e             = INF;
 	        ...
 	        unsigned int tile)
{
  ...
  aux->cc     	= (int **)vrna_alloc( 2        * sizeof(int *));
  aux->Fmi	= (int **)vrna_alloc( tile     * sizeof(int *));
  aux->DMLi	= (int **)vrna_alloc( (tile+2) * sizeof(int *));
  
  for (i = 0; i < 2; i++)
    aux->cc[i]   = (int *)vrna_alloc(sizeof(int) * (length + 2));  /* auxilary arrays for canonical structures     */
  
  for (i = 0; i < tile; i++)
    aux->Fmi[i]	 = (int *)vrna_alloc(sizeof(int) * (length + 1));  /* holds row i of fML (avoids jumps in memory)  */

  for (i = 0; i < (tile+2); i++)
    aux->DMLi[i] = (int *)vrna_alloc(sizeof(int) * (length + 1));  /* DMLi[j] holds  MIN(fML[i,k]+fML[k+1,j])      */
		  ...
		  unsigned int 	    i)
{
  ...
  // Previous values of Fmi are not needed
  for (j = 0; j < tile; j++)
  ...
  // CC is not needed as --noLP option is not used
  //FF          = aux->cc1;
  //aux->cc1    = aux->cc;
  //aux->cc     = FF;
