...
PRIVATE int
E_ml_stems_fast_patch(vrna_fold_compound_t  *fc,
                int                   i,
                int                   j,
		int 		      ii,
		int 		      jj,
...
PUBLIC int
vrna_E_ml_stems_fast_patch(vrna_fold_compound_t *fc,
                     int                  i,
                     int                  j,
		     int 		  ii,
		     int 		  jj,
                     ...
                     int                  *dmli)
{
  ...
  if (fc)
    e = E_ml_stems_fast_patch(fc, i, j, ii, jj, tile, fmi, dmli);
...
PUBLIC int64_t
vrna_E_ml_stems_fast_middle(vrna_fold_compound_t *fc,
		     int 		  ii,
		     int 		  jj,
		     int                  tile,
                     int                  **ha_fmi,
                ...
                int                   *dmli)
{
  ...
  if (sliding_window) {
	...
	//
      	...
      	const int section_down = tile - (jj - j) + 1; 

      	// Patch Top
      	int k_top     	    = k;
      	const int k1j_top   = indx[j] + k_top + 1;
      	const int count_top = section_up;
      	en        = vrna_fun_zip_add_min(fmi_tmp + k_top, fm + k1j_top, count_top);
      	decomp    = MIN2(decomp, en);

      	// Middle  
      	//int k_mid	    = k + section_up;
      	//const int k1j_mid   = indx[j] + k_mid + 1;
      	//const int count_mid = last_nt - k - section_down - section_up;
      	//en        = vrna_fun_zip_add_min(fmi_tmp + k_mid, fm + k1j_mid, count_mid);
      	//decomp    = MIN2(decomp, en);

      	// Patch Bottom
      	int k_bot	    = last_nt - section_down;
      	const int k1j_bot   = indx[j] + k_bot + 1;
      	const int count_bot = section_down;
      	en        = vrna_fun_zip_add_min(fmi_tmp + k_bot, fm + k1j_bot, count_bot);
      	decomp    = MIN2(decomp, en);
      	//
	//
  ...
  decomp = MIN2(dmli[j], decomp); // Considered stored result from the middle phase
